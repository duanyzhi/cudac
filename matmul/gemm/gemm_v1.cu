...
#pragma unroll
  ...
  __syncthreads();
  // const int laneId = threadIdx.x & 31; // warp内线程索引


  // printf("num_frag_a: %d. num_frag_b: %d\n", num_frag_a, num_frag_b);
  ...
  if (tid == 0) {
    ...
    // printf("tid: %d\n. ", tid);
      // for (int a = 0; a < num_frag_a; ++a) {
      //   for (int b = 0; b < num_frag_b; ++b) {
          // printf("tid: %d. num_frag_a: %d.  num_frag_b: %d. frag_c[a][b].num_elements: %d \n", tid, num_frag_a, num_frag_b, frag_c[a][b].num_elements);
          // for (int n =  0; n < frag_c[0][0].num_elements; ++n) {
          //   float vf = frag_c[0][0].x[n];
          //   printf("%f ", vf);
          // }
          // printf("\n");
      //   }

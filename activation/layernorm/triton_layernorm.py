...
torch_base = torch.nn.functional.layer_norm(x, w_shape, weight, bias, eps)

for _ in range(10):
   out = layer_norm(x, w_shape, weight, bias, eps)  # noqa: F811, E704

print(out, torch_base)
print("Triton: ", torch.allclose(torch_base, out))

diff = torch.abs(out - torch_base)
max_abs_error = torch.max(diff)  # 最大绝对误差: 0.02
max_rel_error = torch.max(diff / torch.abs(torch_base))  # 最大相对误差: 0.01
...
print(out)
print("avg pytorch compile layernorm sync time: ", start.elapsed_time(end) / 100, " ms.")
